...
4. **Chuyển đổi AI Provider**: Click icon extension → chọn **Local** hoặc **Cloud** trong phần AI Provider.
5. **Kiểm tra trạng thái**: Click vào icon extension để kiểm tra kết nối AI đang hoạt động.

## 🏗️ Cấu trúc dự án

- `manifest.json`: Cấu hình Extension Manifest V3.
- `background.js`: Xử lý logic gọi API Ollama / Google Gemini ở background.
- `content.js`: Quản lý DOM, bắt sự kiện bôi đen và hiển thị UI dịch.
- `content.css`: Styling cho popup dịch (Glassmorphism).
